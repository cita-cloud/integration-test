...
echo "update-raft..."
docker run -it --rm -v $(pwd):/data -w /data registry.devops.rivtower.com/cita-cloud/cloud-config cloud-config create-k8s --chain-name test-chain-raft --admin 0x9bab5858df4a9e84ff3958884a01a4fce5e07edb --nodelist localhost:40000:node0:k8s,localhost:40001:node1:k8s,localhost:40002:node2:k8s,localhost:40003:node3:k8s --consensus_image consensus_raft --access-key-id admin --secret-access-key testtest --s3-endpoint http://minio:9000 --s3-bucket cita-cloud --service-type s3 --s3-root raft
docker run -it --rm -v $(pwd):/data -w /data registry.devops.rivtower.com/cita-cloud/cloud-config cloud-config append-k8s --chain-name test-chain-raft --node localhost:40004:node4:k8s --access-key-id admin --secret-access-key testtest --s3-endpoint http://minio:9000 --s3-bucket cita-cloud --service-type s3 --s3-root raft
